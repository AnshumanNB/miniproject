...
const experiments = [
  ...
  {
    id: 'exp21',
    label: 'Theory of Computation - TM',
    path: '/labs/experiment22',
    rpath: 'experiment22',
    render: (
      <TuringMachineExperiment />
    ),
  },
...
function TuringMachineExperiment() {
  const STATE_RADIUS = 0.5;
  const HIGHLIGHT_COLOR = 0xffff00;
  ...
  const transitions = {
    // q0: main loop – find first unmarked A, or halt if finished
    q0: {
      X: { next: 'q0', write: 'X', move: 'R' },      // skip processed A
      Y: { next: 'q0', write: 'Y', move: 'R' },      // skip processed B
      A: { next: 'q1', write: 'X', move: 'R' },      // first A of a new pair
      // B: no rule -> reject (extra B or wrong order)
      // '_': no rule in q0 -> ACCEPT (handled in runTM)
    },

    // q1: we already marked one A as X; mark second A as X
    q1: {
      X: { next: 'q1', write: 'X', move: 'R' },      // skip other X's
      A: { next: 'q2', write: 'X', move: 'R' },      // second A -> X
      // anything else -> no rule -> reject (not enough A's)
    },

    // q2: look for one B to match this pair of A's
    q2: {
      X: { next: 'q2', write: 'X', move: 'R' },
      Y: { next: 'q2', write: 'Y', move: 'R' },
      A: { next: 'q2', write: 'A', move: 'R' },      // still in A-region
      B: { next: 'q3', write: 'Y', move: 'L' },      // found B -> mark Y and go back
      // '_': no rule -> reject (no B to match)
    },

    // q3: move back left to the beginning and restart
    q3: {
      X: { next: 'q3', write: 'X', move: 'L' },
      Y: { next: 'q3', write: 'Y', move: 'L' },
      A: { next: 'q3', write: 'A', move: 'L' },
      B: { next: 'q3', write: 'B', move: 'L' },
      '_': { next: 'q0', write: '_', move: 'R' },    // back to start of tape
    },
  };

  /* ---------------------------
       State positions for 3D
       (4 states laid out clearly)
  ----------------------------*/

  const positions = {
    q0: new THREE.Vector3(-3, 1.5, 0),
    q1: new THREE.Vector3(0, 2.5, 0),
    q2: new THREE.Vector3(3, 1.5, 0),
    q3: new THREE.Vector3(0, 0, 0),
  };
  ...
  useEffect(() => {
    ...
    /* Create States as spheres with labels */
    function createState(stateId) {
      const color = stateColors[stateId];
      const geom = new THREE.SphereGeometry(STATE_RADIUS, 32, 32);
      const mat = new THREE.MeshBasicMaterial({ color });
      ...
      sprite.position.set(
        positions[stateId].x,
        positions[stateId].y - STATE_RADIUS * 2,
        positions[stateId].z
      );
      scene.add(sprite);

      return { mesh, outline, sprite };
    }

    Object.keys(positions).forEach((stateId) => {
      const { mesh, outline, sprite } = createState(stateId);
      stateMeshes.current[stateId] = { mesh, outline };
      labelSprites.current[stateId] = sprite;
    });

    /* Draw simple lines between states */
    function drawLine(from, to) {
      const points = [from, to];
      const geometry = new THREE.BufferGeometry().setFromPoints(points);
      const material = new THREE.LineBasicMaterial({ color: 0xffffff });
    ...
    drawLine(positions.q0, positions.q1);
    drawLine(positions.q1, positions.q2);
    drawLine(positions.q2, positions.q3);
    drawLine(positions.q3, positions.q0);
  ...
  useEffect(() => {
    if (!currentState || !stateMeshes.current[currentState]) return;
    Object.values(stateMeshes.current).forEach((entry) => {
      if (!entry || !entry.outline) return;
      entry.outline.material.opacity = 0;
    });
    const entry = stateMeshes.current[currentState];
    if (entry && entry.outline) {
      entry.outline.material.opacity = 1.0;
    }
  }, [currentState]);
  ...
  ----------------------------*/
  const runTM = () => {
    if (isRunning || !inputString) return;
    setIsRunning(true);
    setTransitionLog([]);
    setFinalResult(null);

    // Tape: blanks on both sides
    let tape = ['_', ...inputString.toUpperCase().split(''), '_'];
    let head = 1; // start at first input symbol
    let state = 'q0';
    ...
    setTimeout(() => {
      const step = () => {
        ...
        if (!rule) {
          // Halting condition: accept only if we are in q0 and see blank
          if (state === 'q0' && symbol === '_') {
            setFinalResult('ACCEPTED');
          } else {
            setFinalResult('REJECTED');
          }
        ...
        setTransitionLog((log) => [
          ...log,
          `${stepIndex}) δ(${state}, ${symbol}) → (${rule.next}, ${rule.write}, ${rule.move})`,
        ]);
  ...
  return (
    <div>
      <h2 style={{ color: 'white' }}>
        Turing Machine – 2×A = B (A*B*, 4 states)
      </h2>
      ...
      <input
        value={inputString}
        onChange={(e) => setInputString(e.target.value)}
        placeholder="Enter string like AAAABB (A*B*)"
        disabled={isRunning}
        style={{ padding: 6, fontSize: 16, marginRight: 8 }}
      />
      ...
      >
        {isRunning ? 'Running...' : 'Run Turing Machine'}
      </button>
      ...
      >
{`A Turing machine is an abstract model of a computer with an infinite tape and a head that reads/writes symbols and changes state according to a fixed set of rules. For the language \(L = \{a^{2n}b^n \mid n \ge 1\}\), one can design a Turing machine that repeatedly “pairs” two \(a\)’s with one \(b\) by marking them on the tape.

## Turing machine basics

A Turing machine has an infinite tape (memory), a read–write head that moves left or right, and a finite set of states including start, accept, and reject.  On each step it looks at (state, current symbol), then writes a symbol, moves the head (L/R), and goes to a new state according to its transition function.

## Language \(a^{2n}b^n\)

Strings in \(L\) are of the form “some even number of \(a\)’s, then half as many \(b\)’s”, e.g. \(aab\), \(aaaabb\), \(aaaaaabbb\).  Informally, the machine must check three things: all \(a\)’s come before all \(b\)’s, the count of \(a\)’s is even, and \(\#a = 2\cdot \#b\).

## High-level TM strategy

Think of extra tape symbols \(X\) and \(Y\) as “marked \(a\)” and “marked \(b\)”.

- From the left, find the first unmarked \(a\); change it to \(X\) (this is the 1st \(a\) of a pair).
- Continue right to find the next unmarked \(a\); change it to \(X\); if you hit a \(b\) or blank before finding this second \(a\), reject (odd number of \(a\)’s or wrong order).
- Continue right to find the first unmarked \(b\); change it to \(Y\); if no such \(b\) is found before blank, reject (too many \(a\)’s).
- Move back to the left end and repeat this whole process of “2 \(a\)’s → 1 \(b\)” matching.
- If at some point you find no unmarked \(a\)’s and also no unmarked \(b\)’s, accept; if no unmarked \(a\)’s but some unmarked \(b\)’s remain, reject.

## Quick example: input “aaaabb”

Input \(aaaabb\) is \(a^4b^2\), so it should be accepted with \(n = 2\).  First loop: mark two leftmost \(a\)’s as \(X\), then mark the leftmost \(b\) as \(Y\); second loop: mark the remaining two \(a\)’s as \(X\) and the remaining \(b\) as \(Y\); third pass finds only \(X\)’s and \(Y\)’s and no unmarked \(a\) or \(b\), so the machine accepts.`}
      </pre>
    </div>
  );
}
